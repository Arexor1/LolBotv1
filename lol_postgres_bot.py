...
class Database:
        ...
        with self.connect() as db:
            with db.cursor() as cur:
                cur.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NULL, CURRENT_TIMESTAMP)
    ...
    def update_current(self, discord_id: int, current: Optional[RankedEntry], error: Optional[str] = None):
        with self.connect() as db:
            with db.cursor() as cur:
                if error:
                    cur.execute("UPDATE players SET last_error=%s, updated_at=CURRENT_TIMESTAMP WHERE discord_id=%s", (error, discord_id))
                    return
                current_score = current.score if current else None
                current_rank = current.label if current else "Unranked"
                cur.execute("""
                    ...
                    SET current_score=%s,
                        current_rank=%s,
                        delta=CASE WHEN %s IS NULL THEN NULL ELSE %s - peak_score END,
                        last_error=NULL,
                        updated_at=CURRENT_TIMESTAMP
                    WHERE discord_id=%s
    ...
    def all_players(self):
        with self.connect() as db:
            with db.cursor() as cur:
                cur.execute("""
                    SELECT * FROM players
                    ORDER BY CASE WHEN delta IS NULL THEN 1 ELSE 0 END, delta DESC, display_name ASC
                """)
                return cur.fetchall()
...
def build_leaderboard_embed(rows) -> discord.Embed:
    embed = discord.Embed(title="LoL Peak Delta Leaderboard", description="**Name | Delta from peak elo**", color=discord.Color.blurple())
    if not rows:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        lines.append(f"**{idx}. [{row['display_name']}]({row['opgg_url']})** | `{delta_text}`")
...
class RegisterModal(discord.ui.Modal, title="LoL Elo Registrierung"):
    riot_id = discord.ui.TextInput(label="Riot ID", placeholder="Name#TAG, z. B. Arexor#EUW", required=True, max_length=80)
    opgg = discord.ui.TextInput(label="OP.GG Link oder leer lassen", placeholder="https://www.op.gg/summoners/euw/name-tag", required=False, max_length=200)
    peak_elo = discord.ui.TextInput(label="Peak Elo", placeholder="z. B. Diamond 2 50 oder Master 120", required=True, max_length=80)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            db.upsert_player(interaction.user.id, interaction.user.display_name, f"{game_name}#{tag_line}", game_name, tag_line, opgg_url, peak_score, current)
            await interaction.followup.send(f"Gespeichert: `{game_name}#{tag_line}` | Peak: `{score_to_label(peak_score)}` | Current: `{current.label if current else 'Unranked'}`", ephemeral=True)
...
async def leaderboard(interaction: discord.Interaction):
    await interaction.response.send_message(embed=build_leaderboard_embed(db.all_players()))
...
async def post_or_update_leaderboard():
    ...
    if channel is None:
        return
    embed = build_leaderboard_embed(db.all_players())
